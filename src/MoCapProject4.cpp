/*
 ...
 Date: 10/31/20
 Notes: MoCap Final Project
 Purpose/Description:
 
 This project will be a virtual music box that plays whenever a certain tracked feature moves in circular motion (as if cranking a music box).
...
MusicBoxApp::MusicBoxApp() : mSender(LOCALPORT, DESTHOST, DESTPORT) //initializing our class variables
{
    
}

void MusicBoxApp::setup()
{
    ...
    params.minArea = 500.0f; //ORIGINALLY 200.0f
    params.maxArea = 2000.0f; //ORIGINALLY 1000.0f
...
void MusicBoxApp::mouseDrag( MouseEvent event)
{
...
void MusicBoxApp::mouseDown( MouseEvent event)
{
    curMousePosLastDown = event.getPos();
    isMouseDown = true;
...
void MusicBoxApp::mouseUp( MouseEvent event)
{
    isMouseDown = false;
}

//SENDS WHERE MOUSE IS (NOT USED FOR THIS PROGRAM)
void MusicBoxApp::sendOSC(std::string addr, float x, float y){
    osc:: Message msg;
    msg.setAddress(addr);
...
void MusicBoxApp::sendOSC(std::string addr, float down){
    osc:: Message msg;
    msg.setAddress(addr);
...
//SENDS BLOB INFO
void MusicBoxApp::sendOSC(std::string addr, float ind, float x, float y){
    osc:: Message msg;
    msg.setAddress(addr);
...
void MusicBoxApp::keyDown( KeyEvent event )
{
    if( event.getChar() == '1')
    {
        mUseBackgroundSubtraction = BackgroundSubtractionState::NONE;
...
void MusicBoxApp::blobDetection(BackgroundSubtractionState useBackground = BackgroundSubtractionState::NONE)
{
    if(!mSurface) return;
...
//    }
    ...
    //sending mouse values for now (to test circular motion detection)
    sendOSC(WHERE_OSCADDRESS, (float)curMousePosLastDown.x,(float)curMousePosLastDown.y);
    
    if(isMouseDown)
        sendOSC(DOWN_OSC_ADDRESS, 1);
...
//updating mBlobs<>
void MusicBoxApp::updateBlobList()
{
...
CINDER_APP( MusicBoxApp, RendererGl,
           []( MusicBoxApp::Settings *settings ) //note: this part is to fix the display after updating OS X 1/15/18
           {
